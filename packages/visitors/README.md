...
[npm-downloads-image]: https://img.shields.io/npm/dm/@soidl/visitors.svg?style=flat
[npm-image]: https://img.shields.io/npm/v/@soidl/visitors.svg?style=flat&label=%40soidl%2Fvisitors
[npm-url]: https://www.npmjs.com/package/@soidl/visitors

This package offers various visitors for Soidl IDLs to traverse and manipulate their nodes.
...
This package includes and re-exports the [`@soidl/visitors-core`](../visitors-core/README.md) package which provides the core interfaces and functions to create and compose visitors.

To get a better understanding of visitors and how they work, please refer to the [`@soidl/visitors-core` documentation](../visitors-core/README.md).

In the rest of this documentation, we focus on the high-level visitors that are only available in this package. The main goal of these visitors is to provide a set of specific operations that can be applied to Soidl IDLs — as opposed to the generic primitives provided by the core package.
...
```ts
soidl.update(fillDefaultPdaSeedValuesVisitor(instructionPath, linkables, strictMode));
```
...
```ts
soidl.update(flattenInstructionDataArgumentsVisitor());
```
...
This visitor goes through all `DefinedTypeNodes` and outputs a histogram of how many times each type is used in the Soidl IDL.

```ts
const histogram = soidl.accept(getDefinedTypeHistogramVisitor());
```
...
```ts
soidl.update(setFixedAccountSizesVisitor());
```
...
This visitor adds a new instruction argument to each of the provided instruction names. The new argument is added before any existing argument and marked as a discriminator of the instruction. This is useful if your Soidl IDL is missing discriminators in the instruction data.
...
```ts
soidl.update(transformDefinedTypesIntoAccountsVisitor(['counter', 'escrow']));
```
...
```ts
soidl.update(transformU8ArraysToBytesVisitor());
```
...
```ts
soidl.update(unwrapDefinedTypesVisitor(['counter', 'escrow']));
```
...
```ts
soidl.update(unwrapInstructionArgsDefinedTypesVisitor());
```
...
```ts
soidl.update(unwrapTupleEnumWithSingleStructVisitor());
```
...
```ts
soidl.update(unwrapTypeDefinedLinksVisitor(['[accountNode]counter.data', '[instructionNode]transfer.config']));
```

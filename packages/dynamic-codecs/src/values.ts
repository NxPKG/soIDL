import { SOIDL_ERROR__ENUM_VARIANT_NOT_FOUND, SoidlError } from '@soidl/errors';
import { assertIsNode, bytesTypeNode, isNode, isScalarEnum, pascalCase, ValueNode } from '@soidl/nodes';
import { LinkableDictionary, NodeStack, pipe, recordNodeStackVisitor, visit, Visitor } from '@soidl/visitors-core';
...
export function getValueNodeVisitor(
    ...
    const baseVisitor: Visitor<unknown, ValueNode['kind']> = {
        ...
        visitEnumValue(node) {
            ...
            if (variantIndex < 0) {
                throw new SoidlError(SOIDL_ERROR__ENUM_VARIANT_NOT_FOUND, {
                    enum: node.enum,

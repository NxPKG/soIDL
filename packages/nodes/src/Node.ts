import { SOIDL_ERROR__UNEXPECTED_NODE_KIND, SoidlError } from '@soidl/errors';
import type { GetNodeFromKind, Node, NodeKind } from '@soidl/node-types';
...
export function assertIsNode<TKind extends NodeKind>(
    ...
    if (!isNode(node, kinds)) {
        throw new SoidlError(SOIDL_ERROR__UNEXPECTED_NODE_KIND, {
            expectedKinds: kinds,
            kind: node?.kind ?? null,
            node,
        });

import { SOIDL_ERROR__UNEXPECTED_NESTED_NODE_KIND, SoidlError } from '@soidl/errors';
import type { NestedTypeNode, Node, TypeNode } from '@soidl/node-types';
...
export function assertIsNestedTypeNode<TKind extends TypeNode['kind']>(
    ...
    if (!isNestedTypeNode(node, kinds)) {
        throw new SoidlError(SOIDL_ERROR__UNEXPECTED_NESTED_NODE_KIND, {
            expectedKinds: kinds,
            kind: node?.kind ?? null,
            node,
        });

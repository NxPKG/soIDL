import { SOIDL_ERROR__UNRECOGNIZED_NODE_KIND, SoidlError } from '@soidl/errors';
import { type GetNodeFromKind, type Node, type NodeKind, pascalCase, REGISTERED_NODE_KINDS } from '@soidl/nodes';
...
export function getVisitFunctionName<TNodeKind extends NodeKind>(nodeKind: TNodeKind) {
    if (!REGISTERED_NODE_KINDS.includes(nodeKind)) {
        throw new SoidlError(SOIDL_ERROR__UNRECOGNIZED_NODE_KIND, { kind: nodeKind });
    }

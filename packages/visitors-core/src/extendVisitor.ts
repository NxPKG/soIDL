import { SOIDL_ERROR__VISITORS__CANNOT_EXTEND_MISSING_VISIT_FUNCTION, SoidlError } from '@soidl/errors';
import { GetNodeFromKind, Node, NodeKind, REGISTERED_NODE_KINDS } from '@soidl/nodes';
    ...
    const overriddenFunctions = Object.fromEntries(
        Object.keys(overrides).flatMap(key => {
            ...
            if (!visitor[castedKey]) {
                throw new SoidlError(SOIDL_ERROR__VISITORS__CANNOT_EXTEND_MISSING_VISIT_FUNCTION, {

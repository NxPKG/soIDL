import { SOIDL_ERROR__VISITORS__CANNOT_REMOVE_LAST_PATH_IN_NODE_STACK, SoidlError } from '@soidl/errors';
import { GetNodeFromKind, Node, NodeKind } from '@soidl/nodes';
...
export class NodeStack {
    ...
    public popPath(): NodePath {
        if (this.stack.length <= 1) {
            throw new SoidlError(SOIDL_ERROR__VISITORS__CANNOT_REMOVE_LAST_PATH_IN_NODE_STACK, {

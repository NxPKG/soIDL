import { CodecAndValueVisitors, containsBytes, ReadonlyUint8Array } from '@soidl/dynamic-codecs';
import {
...
function matchFieldDiscriminator(
    ...
    if (!field) {
        throw new SoidlError(SOIDL_ERROR__DISCRIMINATOR_FIELD_NOT_FOUND, {
            field: discriminator.name,
        });
    }
    if (!field.defaultValue) {
        throw new SoidlError(SOIDL_ERROR__DISCRIMINATOR_FIELD_HAS_NO_DEFAULT_VALUE, {
            field: discriminator.name,
        });
    }
